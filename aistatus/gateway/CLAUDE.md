...
| `server.py` | proxy | Serve `/health` `/status` `/usage`, pre-mark globally degraded models, proxy upstream requests, account every response through `usage_accounting`, and hot-reload config via `reload_config()` + an mtime-polling watcher task |
